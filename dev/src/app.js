...
document.getElementById('order-select').addEventListener('change', renderBoxView);
...
function updateSpendingDashboard() {
    ...
    if (listEl) {
        if (purchases.length === 0) {
            listEl.innerHTML = '<p style="font-size:13px; color:#999;">No purchases yet. Visit a landmark or business to log spending!</p>';
        } else {
...
document.getElementById('spend-btn').addEventListener('click', function() {
    ...
    confirm.textContent = '£' + amount.toFixed(2) + ' logged at ' + currentPanelTitle;
...
function renderBoxView() {
    ...
    const data = (currentCategory === 'landmarks' ? landmarksData : businessesData).slice();
